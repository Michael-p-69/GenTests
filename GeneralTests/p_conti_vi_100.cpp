...
namespace GeneralTests{
    ...
    TM_RETURN  contiVI100::Execute(){
        ...
        for(size_t entry = 0; entry < DUTPins.GetSize(); ++entry){
            VI.Force(_get_aliased(DUTPins[entry]),VI_FORCE_I,Forced[entry],Forced[entry]*2,3,-3);
        }
    ...
    PinM contiVI100::pin_redirect(const PinM& alias) const{
        return alias;
    }
    
    PinML contiVI100::_get_aliased(const PinML& pins){
        PinML aliased = "";
...
//         }
    ...
    std::map<REN::UTILS::FlowSequence, std::map<PinM, FloatM>> contiVI100::_delta_map; 
    

//#ifdef ENABLE_REN_GENRIC
TMFAMILY_CLASS(REN::GeneralTests::contiVI100)
//#endif 
